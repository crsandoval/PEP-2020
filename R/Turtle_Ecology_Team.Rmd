...
ggplot(data = dive_bar, aes(x = factor(month), y = avg_dive_dur, fill = factor(day_night))) + geom_bar(position = 'dodge2', stat = 'identity') + xlab('Month') + ylab('Avg Dive Duration (Sec)') +  ggtitle('Day and Night Average Dive Duration') + guides(fill = guide_legend(title = element_blank()))
...
## ggplot barplot of avg. dive duration by month and day / night
ggplot(data = dive_bar_2, aes(x = factor(month), y = avg_dive_dur, fill = factor(dive_cat))) + geom_bar(position = 'dodge2', stat = 'identity') + xlab('Month') + ylab('Avg Dive Duration (Sec)') + ggtitle('Day and Night Average Dive Duration') +guides(fill = guide_legend(title = element_blank())) +
  facet_wrap(~day_night)
```

```{r import-data, echo = FALSE}
surf_data = readr::read_csv(here::here('data', 'te_dive_20200626.csv'))
```

```{r clean-data, echo = FALSE}
## remove empty columns
surf_data %<>% remove_empty(which = 'cols')
## format column names
surf_data %<>% clean_names(case = 'snake')
## convert ds_date and de_date to datetimes
surf_data %<>% mutate(ds_date = as.POSIXct(ds_date, format = '%d-%b-%y %H:%M:%S', tz = 'UTC'),
                      de_date = as.POSIXct(de_date, format = '%d-%b-%y %H:%M:%S', tz = 'UTC'))
## try to calculate ds_date if missing
surf_data %<>% mutate(ds_date = if_else(is.na(ds_date), de_date - surf_dur, ds_date))
## calculate time at depth
surf_data %<>% mutate(time1 = t1 / 100 * surf_dur + ds_date,
                      time2 = t2 / 100 * surf_dur + ds_date,
                      time3 = t3 / 100 * surf_dur + ds_date,
                      time4 = t4 / 100 * surf_dur + ds_date,
                      time5 = t5 / 100 * surf_dur + ds_date
                      )
## add start / end times
surf_data %<>% mutate(d0 = 0, d6 = 0, time0 = ds_date, time6 = de_date)
## create an id column that tracks dives by individual
surf_data %<>% group_by(newptt) %>% arrange(ds_date) %>% mutate(id = 1:n()) %>% ungroup()
```

```{r data-modify, echo = FALSE}
## add a column to identify day / night dives (6am - 8pm)
surf_data %<>% mutate(ds_date_local = with_tz(ds_date, tz = 'America/New_York'),
                      de_date_local = with_tz(de_date, tz = 'America/New_York'),
                      start_day = paste0(format(ds_date, '%Y-%m-%d'), ' 06:00:00') %>% as.POSIXct(., format = '%Y-%m-%d %H:%M:%S', tz = 'America/New_York'), 
                      end_day = paste0(format(ds_date, '%Y-%m-%d'), ' 19:59:59') %>% as.POSIXct(., format = '%Y-%m-%d %H:%M:%S', tz = 'America/New_York'),
                      day_night = if_else(ds_date_local >= start_day & ds_date_local <= end_day, 'Day', 'Night')
...
## add a column to identify dive categories
surf_data %<>% mutate(surf_cat = case_when(max_dep >= 0 & max_dep < 6 ~ 'Shallow',
                                           max_dep >= 6 & max_dep < 30 ~ 'Midwater',
                                           max_dep >= 30 & max_dep < 70 ~ 'Deep',
                                           max_dep >= 70 ~ 'Very Deep',
                                           TRUE ~ as.character(max_dep)),
                      surf_cat = factor(surf_cat, levels = c('Shallow', 'Midwater', 'Deep', 'Very Deep')
                      ))
```

```{r data-errors}
## time must be sorted
error_1 = surf_data %>% dplyr::select(newptt, id, time0, time1, time2, time3, time4, time5, time6) %>% pivot_longer(cols = starts_with('time'), names_to = 'order', values_to = 'time') %>% group_by(newptt, id) %>% arrange(order, .by_group = TRUE) %>% summarise(error = is.unsorted(time)) %>% ungroup()
## join error column back into dive_data (main dataset)
surf_data %<>% left_join(error_1, by = c('newptt', 'id'))
## take a look
surf_data %>% filter(error) %>% dplyr::select(time0, time1, time2, time3, time4, time5, time6)
## remove potentially erroneous data
surf_data %<>% filter(!error)
## depth must be >= 0
surf_data %>% dplyr::select(d0, d1, d2, d3, d4, d5, d6) %>% filter_all(all_vars(. > 0))
## get rid of records where ds_date is a decade or more earlier than de_date (weird)
surf_data %<>% filter(!(newptt %in% c(118944, 118947, 118887, 118894, 117167, 129780, 118910) & id == 1))
```

```{r plot-summaries}
## extract month and year
surf_data %<>% mutate(month = format(ds_date, '%m') %>% as.numeric(), year = format(ds_date, '%Y') %>% as.numeric())
## average dive duration by month and day / night
surf_bar = surf_data %>% group_by(month, day_night) %>% summarise(avg_surf_dur = mean(surf_dur)) %>% ungroup()
## ggplot barplot of avg. dive duration by month and day / night
ggplot(data = surf_bar, aes(x = factor(month), y = avg_surf_dur, fill = factor(day_night))) + geom_bar(position = 'dodge2', stat = 'identity') + xlab('Month') + ylab('Avg Surf Duration (Sec)') + ggtitle('Day and Night Average Surface Duration')  + guides(fill = guide_legend(title = element_blank()))
## average dive duration by month, dive category, and day / night
surf_bar_2 = surf_data %>% group_by(month, surf_cat, day_night) %>% summarise(avg_surf_dur = mean(surf_dur)) %>% ungroup()
## ggplot barplot of avg. dive duration by month and day / night
ggplot(data = surf_bar_2, aes(x = factor(month), y = avg_surf_dur, fill = factor(surf_cat))) + geom_bar(position = 'dodge2', stat = 'identity') + xlab('Month') + ylab('Avg Surf Duration (Sec)') + ggtitle('Day and Night Average Surface Duration')  +guides(fill = guide_legend(title = element_blank())) +
  facet_wrap(~day_night)
```
...
```{r}
plt_full_map = ggplot(dive_data, aes(lon,lat)) + 
    geom_path(color = 'blue') +
    geom_polygon(aes(long, lat, group=group), data = map_data('world'))
```
```{r}
...
```
```{r}
plt_map = ggplot(dive_data, aes(lon,lat)) + 
    geom_path(color = 'blue') +
    geom_polygon(aes(long, lat, group=group), data = map_data('world')) +
...
```{r}
plt_full_map_100120 = ggplot(dive_data %>% filter(newptt == 100120), aes(lon,lat)) + 
    geom_path(color = 'blue') +
    geom_polygon(aes(long, lat, group=group), data = map_data('world'))
```
...
```{r}
plt_map_100120 = ggplot(dive_data %>% filter(newptt == 100120), aes(lon,lat)) + 
    geom_path(color = 'blue') +
    geom_polygon(aes(long, lat, group=group), data = map_data('world')) +
  coord_quickmap(xlim = c(-75, -20), ylim = c(12,60))
```
...
#p1 = longitude and latitude points. Can be vector of two numbers, a matrix of 2 columns (first column is longitude, second column is latiude) ir a spatialPoints object
...
#p2 = As above; or missing, in which case the sequential distance between the points in p1 is computed 
```{r}
dive_data %<>% mutate(horizontal_displacement = distGeo(p1 = c(lon,lat), p2 = c(-75,20)) / 1000)
```
